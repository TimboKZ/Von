...
class Von {
    ...
    static prepareOptions(rawOptions) {
        ...
        return Von.validateOptions(rawOptions)
            ...
            .then(config => options = Object.assign(options, config))
            .then(() => options);
    }
    ...
    static validateOptions(rawOptions) {
        ...
        // TODO: Duplicate some of these to the `loadConfig` checks
        ...
        return Promise.resolve()
            ...
            .then(config => {
                let joiSchema = Joi.object().required().keys({
                    ...
                    autoGroup: Joi.bool().default(true),
                    groups: Joi.array().default([]),
                });
                let result = Joi.validate(config, joiSchema);
                if (result.error) throw new Error(`Invalid config: ${result.error}`);
                return result.value;
            });
    }
